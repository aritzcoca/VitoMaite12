...
function mostrarDetallesUsuario(usuario) {
    ...
    solicitud.onsuccess = function (evento) {
        ...
        obtenerAficiones(db, usuario.email, function (aficiones) {
            mostrarVentana(usuario, aficiones);
        });
    };
}

// Obtener aficiones del usuario
function obtenerAficiones(db, email, callback) {
    var transaccion = db.transaction(["AficionesUsuarios", "Aficiones"], "readonly");
    var aficionesUsuariosStore = transaccion.objectStore("AficionesUsuarios");
    var aficionesStore = transaccion.objectStore("Aficiones");

    var cursor = aficionesUsuariosStore.index("email").openCursor(IDBKeyRange.only(email));
    var idsAficiones = [];
    ...
    cursor.onsuccess = function (eventoCursor) {
        ...
        if (resultado) {
            idsAficiones.push(resultado.value.aficion);
            resultado.continue();
        } else {
            var nombresAficiones = [];
            if (idsAficiones.length === 0) {
                callback(nombresAficiones);
                return;
            }

            idsAficiones.forEach(function (idAficion) {
                var solicitudAficion = aficionesStore.get(idAficion);

                solicitudAficion.onsuccess = function (eventoAficion) {
                    nombresAficiones.push(eventoAficion.target.result.nombre);
                    if (nombresAficiones.length === idsAficiones.length) {
                        callback(nombresAficiones);
                    }
                };
            });
...
// Mostrar ventana con información y aficiones
function mostrarVentana(usuario, aficiones) {
    var overlay = document.createElement("div");
    overlay.id = "overlay";
    overlay.style.position = "fixed";
